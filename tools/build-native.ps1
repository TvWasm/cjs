...
foreach ($abi in @('armeabi-v7a','arm64-v8a')) {
  ...
  $abiOutput = Join-Path $projectRoot "build\libs-$abi"
  ...
  $builtAbi = Join-Path $outputRoot $abi
  $savedAbi = Join-Path $abiOutput $abi
  if (Test-Path -LiteralPath $savedAbi) { Remove-Item -Recurse -Force -LiteralPath $savedAbi }
  New-Item -ItemType Directory -Force -Path $abiOutput | Out-Null
  Copy-Item -Recurse -Force -LiteralPath $builtAbi -Destination $savedAbi
}
foreach ($abi in @('armeabi-v7a','arm64-v8a')) {
  $savedAbi = Join-Path (Join-Path $projectRoot "build\libs-$abi") $abi
  $targetAbi = Join-Path $outputRoot $abi
  if (Test-Path -LiteralPath $targetAbi) { Remove-Item -Recurse -Force -LiteralPath $targetAbi }
  Copy-Item -Recurse -Force -LiteralPath $savedAbi -Destination $targetAbi
}
